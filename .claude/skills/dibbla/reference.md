...
| **Storage** | The list is a plain, editable YAML file at `~/.config/dibbla/config.yaml`, holding **no secrets**. Tokens stay in the OS keyring under per-context keys, falling back to a per-context `~/.config/dibbla/credentials.<name>.env` on hosts without a keyring. A file rather than keyring-only because no OS offers a portable "enumerate everything under this service" API, so `context list` could not otherwise be trusted. Each context also records `store: keyring\|file` — which store its token actually went to — so a keyring-less host stops looking for a keyring on every command. |
| **Which store, on Linux** | macOS (Keychain) and Windows (Credential Manager) always use the OS store. On Linux the CLI first checks whether a D-Bus session bus is addressable at all (`$DBUS_SESSION_BUS_ADDRESS`, or `/run/user/<uid>/bus`). **No session bus → the file store, without contacting D-Bus.** That is the normal state on a headless server: nothing there implements `org.freedesktop.secrets`, and probing would only fork a `dbus-daemon` before failing. With a session bus the keyring is tried, time-boxed at 2s for a read and 10s for a write; a timeout or any error falls back to the file. A keyring prompt the user actively *dismisses* is kept as a failure — that is a decision, not a missing keyring. |
| **Overriding it** | `DIBBLA_CREDENTIAL_STORE=keyring\|file\|auto` (default `auto`). Set `file` to skip the keyring on a host where one is reachable but unwanted; set `keyring` to force it where the probe guesses wrong. An unrecognised value is an error from `dibbla login` and `dibbla status` rather than a silent fall back to `auto`. |
...
|  | **Auto-fallback** (no flag) — when the OS keyring is unavailable (a headless Linux host: no session bus, or nothing providing `org.freedesktop.secrets`), `dibbla login` writes credentials to a per-context file at `~/.config/dibbla/credentials.<name>.env` (mode 0600), and mirrors the context in use to the legacy `~/.config/dibbla/credentials.env`. Both are read regardless of cwd, mirroring keychain semantics. `dibbla logout` and `dibbla uninstall` clean them up. The login output names the file **and** says the token is stored in plaintext; see the security note below. |
|  | **Security of the file store** — the file is mode 0600 in a 0700 directory and the token in it is **not encrypted**. That keeps out other users on the host; it does not keep out root, a backup or disk image, or anything running as the same user (a postinstall script, a CI step, a dependency in a repo you `cd` into). There is no encrypted-at-rest option on a headless host: any key the CLI could decrypt with unattended would have to sit next to the file. **For a server, prefer not persisting at all** — supply `DIBBLA_API_TOKEN` per invocation from systemd `LoadCredential=`, a secrets manager or the CI store, which bypasses every local store (see Resolution order). |
...
| Cloud VM / SSH / Docker (no keyring), CLI ≥ v1.2.21 | `dibbla login --api-key=<t> --api-url=<url>` (auto-falls back to user-level file, plaintext 0600) |
| Long-lived server, nothing persisted on disk | `DIBBLA_API_TOKEN` in the unit's environment (systemd `LoadCredential=`) — no `dibbla login` at all |
...
| **Linked folder** | Run inside a folder connected with `dibbla clone` / `dibbla link` (any subdirectory), `status` adds `Folder:` (repo root), `App:` (`<app>  (org <slug>, remote origin)`, read from the remote URL), `Sync:` (`in sync with Dibbla`, `N commit(s) ahead of Dibbla`, `N commit(s) behind Dibbla — git pull to catch up`, or `N ahead, M behind — histories have diverged`; `; uncommitted changes on disk` when dirty) and `Running:` (`✅ <sha> — the commit the app runs is this folder's HEAD`, `⚠ <sha> — not this folder's HEAD (<local>)`, or `unknown`). The counts follow a `git fetch`; `--no-validate` skips the fetch and the running-commit lookup and says `(as of the last fetch)`. In `--json` this is the `folder` object (`app`, `org`, `remote`, `branch`, `head_sha`, `ahead`, `behind`, `fetched`, `dirty`, `running_sha`, `running_is_head`); absent outside a linked folder. |
| **Exit codes** | `0` — logged in, or `--no-validate` and a token is configured. `3` — not logged in / token rejected. `1` — unexpected error (network, malformed response). The folder lines never change the exit code. |
...
Plan:    standard
Folder:  /Users/me/projects/my-app
App:     my-app  (org acme, remote origin)
Sync:    1 commit(s) ahead of Dibbla
Running: ⚠ 3f2a9c1 — not this folder's HEAD (8b1c2d3)
Status:  ✅ logged in
```
...
| **Linked folder (git mode)** | In a folder linked to its app (`dibbla clone` / `dibbla link`; the deploy root is the repo root and a remote points at Dibbla) `deploy` never sends a tarball — it is sugar for commit + push so the history stays one: everything tracked and untracked (`.gitignore` honoured) is committed with `-m` as the subject, `HEAD` is pushed to `main`, and the deploy the push started is followed to the end (same output and exit code as `dibbla deploy status --follow`). `-m` is required whenever there is something to commit. **`--update`** is the default and needs no flag: a push is always a rolling update. **`--force`** means *redeploy even though nothing changed*: with local `HEAD` = Dibbla's `main` the CLI otherwise says `Nothing new to deploy` (exit 0); `--force -m "redeploy: …"` records an empty commit and pushes it, so the redeploy is visible in the history. Dibbla's `main` ahead of the folder (an MCP patch, another machine) → `fetch first: … run git pull --rebase origin main` (exit 1, nothing deployed, the local change is already committed). `--alias` must name the linked app; `--env`, `--env-file`, `--cpu`, `--memory`, `--port`, `--favicon`, `--require-login`, `--access-policy`, `--google-scopes`, `--microsoft-scopes`, `--target-env`, `--profile`, `--no-public` are refused (the push deploys with the app's stored configuration — `dibbla apps update` or `dibbla.yaml`). A folder without git, a repo with only a GitHub remote, or a subdirectory of a linked repo takes the tarball path exactly as before. |
...
Connect a local folder to the Dibbla-managed git repo of a deployed app. Every `dibbla deploy` writes one commit to a platform-managed bare repo (subject = the deploy's `-m` message, `Deploy-Id:` trailer = the deployment). `clone` fetches that history so you — or a coding agent on a machine that has never seen the code — can pick up exactly what is running. Dibbla is the app's `origin`; the command **looks at the folder first and does what fits**, so never run `git clone` by hand and never answer `destination path '.' already exists` by working around it.

| Item | Details |
|------|---------|
| **Usage** | `dibbla clone <app> [--into <dir>]` or `dibbla link <app>` (= `clone --into .`, link the folder you are in). `<org>/<app>` is accepted; the org still comes from the active context. |
| **Flags** | `--into <dir>` — destination (default `./<app>`; `link` defaults to `.`). `--yes` / `-y` — link a folder that has its own git history without the prompt (agents: always pass it when you mean it). `--ref <sha>` — fresh clones only: commit to check out after cloning. |
| **Folder states** | *Empty or missing* → `git clone`; origin = Dibbla, `main` checked out. *Files but no `.git`* → without `--into` the app is cloned into `./<app>/` and the CLI says so; with `--into .` the folder is linked in place (files kept). *A repo with no commits* (Codex's `git init`) → remote added, `main` fetched and checked out; no git clone error. *A repo with its own history and no Dibbla remote* → the CLI explains that the files on disk are kept but the history starts over from Dibbla's `main`, asks for confirmation (`--yes` for agents), moves the old commits to a `pre-dibbla-<timestamp>` branch, and leaves `git status` showing disk vs Dibbla as changes to commit. Two histories are never merged. *Already linked to this app* → `git pull --ff-only`, "already linked — updated". `dibbla clone <app>` with no `--into` in a folder already linked to that app does the same. |
| **Existing `./<app>`** | If `./<app>` exists and is not a fresh clone target (files, or a repo), `clone` without `--into` refuses with a hint: `--into ./<app>` to link it in place, or `--into <other>`. Nothing is touched. |
| **Behavior** | 1. Requires a token (`dibbla login` or `DIBBLA_API_TOKEN`) and `git` on `PATH`. 2. Resolves the canonical clone URL with `GET /api/deploy/deployments/<app>/vcs/info`. 3. git authenticates through the `dibbla git-credential` helper that `dibbla login` registers for Dibbla's git host (`credential.https://<api-host>/git.helper`) — the token never lands in `.git/config`, the remote URL or `~/.git-credentials`, and `git pull`/`git fetch` in the folder use the same login. 4. If `origin` already points somewhere else (GitHub), Dibbla is added as remote `dibbla` and the CLI says so. |
| **After linking** | `dibbla status` in the folder shows the app, org, `N commit(s) ahead / behind Dibbla` and whether the commit the app runs is this folder's HEAD. |
| **Delivering changes** | `dibbla deploy . --alias <app> -m "…" --update` from the folder — the repo is written by deploys, and that is the app's version history. `git push` is answered with `403 push is not permitted; use the deploy pipeline` **by design**; do not try it, and do not answer the 403 by proposing a GitHub/GitLab remote. You may commit locally as much as you like; those commits stay on your machine and are not the delivery step. |
| **What is in the clone** | Exactly the files the deploy uploaded, minus the server-side VCS filter (`.env`, `.env.*`, `node_modules/`, `dist/`, `.venv/`, `.git/`, `*.pem`, `*.key`, plus anything in `.dibblaignore`). So: no secrets or env files — they live on the platform and are already attached to the app. No original git history either: the commits are Dibbla's deploy commits. |
| **What is NOT in the clone** | Anything that was never deployed: uncommitted or undeployed edits on the machine that last deployed, local branches, build outputs. |
| **Output** | `✅ Cloned to <dir>  (origin = Dibbla, main checked out)` / `✅ Linked <dir> to <app>  (remote origin = Dibbla, main tracks origin/main)` / `✅ <dir> is already linked to <app> — updated to Dibbla's main (git pull --ff-only).`, each followed by `latest: <short-sha>  <subject>`. A link also reports files checked out from Dibbla and the `pre-dibbla-*` branch when there was history. With no deploy-written commits yet: `⚠️ Nothing to clone: the app has no deploy-written commits yet.` (exit 0). |
| **Errors** | Not logged in → `API token is required. Run 'dibbla login' or set DIBBLA_API_TOKEN.` (exit 1). Token rejected → `Authentication failed. …` (401). `Access denied for organization <name> (<id>). If the app lives in another organization: dibbla org list, then dibbla org use <name>` (403) and `App not found in organization <name> (<id>) (or version control is not enabled for this environment). …` (404) — both name the org the CLI acted as, because a wrong org pin is the usual cause. Own history without `--yes` on a non-TTY → `not linked. Re-run with --yes to link without a prompt` (exit 1, nothing changed). Already linked but diverged → `git pull --ff-only failed`; see `dibbla status`. |
