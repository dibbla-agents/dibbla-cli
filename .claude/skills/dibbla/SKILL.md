...
| Org        | `org list` (organizations you belong to, active one marked), `org use <name\|slug\|id>` (switch; stored machine-wide, no re-login needed), `org clear` (back to the account default). `--org <id>` is a global flag on every command for a one-off switch; `DIBBLA_ORG_ID` does the same per shell. Precedence: `--org` > `DIBBLA_ORG_ID` > stored > account default |
| Status     | `status` (CLI version + resolved API URL with source + token presence with source + active organization with source + live token validation), `status --no-validate` (skip the network call), `status --json` (machine-readable). Exits `0` when logged in or token is configured + validation skipped, `3` when not logged in or token rejected |
