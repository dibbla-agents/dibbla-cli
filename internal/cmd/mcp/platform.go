...
THE SHAPE OF THE TOOLSET. A full write grant lists 32 tools for everything the
...
and then platform_destructive_execute. Your organization's own exposed
functions are one tool too, platform_tools, with list/invoke/invocations for
everyone and expose/unexpose for owners and admins ('dibbla fn expose' is the
same policy from a shell). A grant sees only the tools its scopes carry, and
on a merged tool only the steps its scopes allow — an action you cannot see is
not missing, it is not yours.
