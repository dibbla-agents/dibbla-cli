...
| MCP        | `mcp community [--client claude\|codex\|gemini]` — print ready-to-paste MCP client config for the hosted community toolset (`mcp.dibbla.com/community`, Dibbla community over MCP acting as you; staff-oriented). Output references `$DIBBLA_API_TOKEN`, never the literal token. `mcp community --check` verifies the whole chain (token → initialize → whoami) and is the first thing to run when a client shows 401 — the usual cause is the env var not reaching the client's process. Reads need one prior sign-in at community.dibbla.com; posting needs write-group membership |
